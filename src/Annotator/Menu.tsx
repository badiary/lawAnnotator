...
const Menu = (props: MenuProps) => {
  ...
  const onChangeLightness = (e: any) => {
    ...
    if (law.sat.word && law.sat.cv) {
      if (e.target instanceof HTMLInputElement) {
        law.sat.word.lightness = Number(e.target.value);
        law.sat.word.setOption(getWordOption());
        setColoredQuery();
      }
    }
  };
    ...
    showSpinner("ワード反転中...", 10, () => {
      if (law.sat.word && law.sat.cv) {
        if (e.target instanceof HTMLInputElement && !blockMode) {
          law.sat.word.block_mode = true;
        } else {
          law.sat.word.block_mode = false;
        }
      }
    });
  };
        ...
        showSpinner("ワード反転中...", 10, () => {
          if (law.sat.word && law.sat.cv) {
            ...
            props.setTextHighlighterOption({
              ...law.sat.word.textHighlighterOption,
  ...
  function getWordOption() {
    ...
    // 反転ワード情報を更新
    const colors = law.sat.word!.getWordColors(word_arr.length);
  ...
  function setColoredQuery() {
    ...
    Object.keys(law.sat.word!.option).forEach((color_id) => {
      law.sat.word!.option[color_id]!.words.forEach(
        (word: string, j: number) => {
          ...
          if (j !== law.sat.word!.option[color_id]!.words.length - 1) {
          ...
          <input
            ...
            onChange={(e) => {
              ...
              reader.onload = function (ev) {
                //テキストエリアに表示する
                props.setLabel(JSON.parse(reader.result as string));
              };
            }}
          />
        </li>
        <li>
          <a href="#" onClick={props.saveLabel}>
            <img
